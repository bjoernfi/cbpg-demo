...
public class PwdChangeLDAPMapper extends AbstractLDAPStorageMapper {
    private static final Logger LOGGER = Logger.getLogger(PwdChangeLDAPMapper.class);
    ...
    public PwdChangeLDAPMapper(ComponentModel mapperModel, LDAPStorageProvider ldapProvider) {
    ...
    public void onImportUserFromLDAP(
        ...
        if (lastPwChangeOld != null && !lastPwChangeOld.equals(lastPwChangeNew)) {
            ...
            LOGGER.info(String.format("Invalidated sessions for user %s (id %s)", user.getUsername(),
                    user.getId()
            ));
        }
