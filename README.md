...
* Sample configuration for **Keycloak** and **OpenLDAP**, serving as the authentication backend(s) for pseudonym generation. The configuration also includes a Keycloak extension that invalidates sessions on external password changes. This extension implements the [SPI](https://www.keycloak.org/docs/latest/server_development/#_providers) provided by Keycloak and is required when using LDAP to synchronize users. 
